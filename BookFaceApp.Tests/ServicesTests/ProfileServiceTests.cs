...
namespace BookFaceApp.Tests.ServicesTests
{
    public class ProfileServiceTests
    {
        ...
        [Test]
        public async Task GetProfilePicShouldWorkCorrectly()
        {
            var service = serviceProvider.GetService<IProfileService>();
            var repo = serviceProvider.GetService<IRepository>();
            ...
            Assert.DoesNotThrowAsync(async () => await service!.GetProfilePictureAsync(userId));

            var profilePictureToString = await service!.GetProfilePictureAsync(userId);
            var pic = await repo!.AllReadonly<ProfilePicture>()
                .FirstOrDefaultAsync(pp => pp.Id == picId);

            Assert.IsNotNull(profilePictureToString);
            Assert.IsNotNull(pic);
            Assert.IsTrue(profilePictureToString == pic.ImageToString);
        }
